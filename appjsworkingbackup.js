...
const loadGoogleFonts = () => {
  ...
  link.href = 'https://fonts.googleapis.com/css2?family=Bangers&family=Dancing+Script:wght@700&family=Lobster&family=Pacifico&family=Fredoka+One&family=Roboto:wght@400;700&display=swap';
  link.rel = 'stylesheet';
...
function App() {
  ...
  const [settings, setSettings] = useState({
    power: 40, speed: 5000, invert: false, dither: false, enableLineArt: false, bgRemovalMethod: 'none',
    // --- MASK SHAPE STATE ADDED HERE ---
    maskShape: 'none', // 'none', 'circle', 'square'
  ...
  const confirmAssetCrop = () => {
      ...
      const outputWidth = containerRect.width * scaleX;
      const outputHeight = containerRect.height * scaleX;
      ...
      ctx.drawImage(
          imgEl, 
          Math.max(0, relativeX * scaleX), 
          Math.max(0, relativeY * scaleX), 
          containerRect.width * scaleX, 
          containerRect.height * scaleX, 
  ...
  const generatePreview = () => {
    ...
    img.onload = () => {
      ...
      // 2. Apply Clipping Mask (Circle/Square) if selected
      ctx.save();
      if (settings.maskShape !== 'none') {
          ctx.beginPath();
          if (settings.maskShape === 'circle') {
              // Draw Circle at center
              ctx.arc(resolution / 2, resolution / 2, resolution / 2, 0, Math.PI * 2);
          } else if (settings.maskShape === 'square') {
              // Draw Square border (redundant if full, but useful if we add margins later)
              ctx.rect(0, 0, resolution, resolution);
          }
          ctx.clip(); // Restricts drawing to this shape
      }
      // --- MASKING LOGIC END ---

      ctx.drawImage(img, 0, 0, resolution, resolution);
      
      ctx.restore(); // Restore context to remove clip for subsequent filters
  ...
  const styles = {
    ...
    modalContent: { background: theme.card, padding:'20px', borderRadius:'16px', width:'90%', maxWidth:'400px', maxHeight:'80vh', display:'flex', flexDirection:'column', gap:'15px', alignItems:'center', boxSizing: 'border-box', overflowY: 'auto' },
          ...
          <div style={styles.modalOverlay}>
              <div style={styles.modalContent}>
                  ...
                  {!assetSourceMode ? (
                      ...
                      <div style={{width:'100%', height:'300px', position:'relative', background:'black', overflow:'hidden'}}>
                          <img ref={assetCropRef} src={previewAssetUrl || tempAssetImage} alt="Crop" style={{position:'absolute', top:'50%', left:'50%', transform: `translate(-50%, -50%) translate(${cropState.x}px, ${cropState.y}px) scale(${cropState.zoom})`, width:'100%', cursor:'move'}} 
                  ...
                  {assetSourceMode === 'crop' && (
                    <div style={{width:'100%', background: theme.buttonSec, padding:'10px', borderRadius:'8px', boxSizing:'border-box'}}>
                      ...
                      {!activeLayer ? (
                        ...
                        <>
                          ...
                          {activeLayer.type === 'text' ? (
                              <>
                                  <textarea rows={2} value={activeLayer.text} onChange={(e) => updateLayer('text', e.target.value.toUpperCase())} style={{width:'100%', padding:'12px', fontSize:'1.2rem', textAlign:'center', borderRadius:'8px', border:`2px solid ${theme.accent}`, background: theme.bg, color: theme.text, fontFamily: activeLayer.fontFamily}} />
                              ...
                              <>
                                  ...
                                  <div>
                                      ...
                                      <input type="range" min="10" max="1500" value={activeLayer.width} onChange={(e) => { const newWidth = parseInt(e.target.value); updateLayer('width', newWidth); updateLayer('height', newWidth * activeLayer.aspectRatio); }} style={styles.slider} />
                                  </div>
                              </>
                          )}
              ...
              {step === 'preview' && (
                <div style={{width: '90%'}}>
                    ...
                    {estimatedTime && ( <div style={{textAlign:'center', background: theme.buttonSec, padding:'10px', borderRadius:'8px', marginBottom:'10px', border:`1px solid ${theme.accent}`}}><span style={{fontWeight:'bold', color: theme.text}}>⏱️ Est. Time: {estimatedTime}</span></div> )}
                      ...
                      <details open={!isMobile}>
                        ...
                        <div>
                            ...
                            {/* --- ADDED UI SELECTOR FOR MASK --- */}
                            <div style={styles.infoBox}>
                                <label style={styles.infoLabel}>Engraving Shape:</label>
                                <select value={settings.maskShape} onChange={(e) => updateSetting('maskShape', e.target.value)} style={styles.select}>
                                    <option value="none">Standard Rectangle</option>
                                    <option value="circle">Circular Keychain</option>
                                    <option value="square">Exact Square</option>
                                </select>
                            </div>
                    ...
                    <details open={!isMobile}>
                        ...
                        <div><label style={styles.label}>Power: {settings.power}%</label><input type="range" min="1" max="100" value={settings.power} onChange={(e) => updateSetting('power', parseInt(e.target.value))} style={styles.slider} /><label style={styles.label}>Speed: {settings.speed}</label><input type="range" min="500" max="5000" step="100" value={settings.speed} onChange={(e) => updateSetting('speed', parseInt(e.target.value))} style={styles.slider} /><label style={styles.label}>Res (L/mm): {settings.linesPerMm}</label><input type="range" min="1" max="10" step="1" value={settings.linesPerMm} onChange={(e) => updateSetting('linesPerMm', parseInt(e.target.value))} style={styles.slider} /></div>
                    </details>
